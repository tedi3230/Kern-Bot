...
def generateEmbed(messageAuthor,color,title,description,imageURL,footerText):
    ...
    embed.set_author(name=messageAuthor)
    return embed
...
async def set_config(ctx,receiveChannel, allowChannel, sendChannel):
    receiveChannel,allowChannel,sendChannel = receiveChannel.split('<')[1].
    setServerChannels(ctx.message.channel.server.id,receiveChannel, allowChannel, sendChannel)
